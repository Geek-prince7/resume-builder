...
async def generate_resume(req: GenerateRequest):
    ...
    # Provider SDKs are synchronous; keep them off the event loop so health
    # checks and other requests remain responsive during long generations.
    result = await asyncio.to_thread(
        generate_tailored_resume, req.user_profile, req.job_description
    )
