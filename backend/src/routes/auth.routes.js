...
router.get('/export', authenticate, async (req, res) => {
  const [jobDescriptions, profileVariants, usageEvents, connectionRequests, jobRecommendations] = await Promise.all([JobDescription.find({ userId: req.user.userId }), ProfileVariant.find({ userId: req.user.userId }), UsageEvent.find({ userId: req.user.userId }), ConnectionRequest.find({ userId: req.user.userId }), JobRecommendation.find({ userId: req.user.userId })]);
  res.set('Content-Disposition', 'attachment; filename="resumeai-data.json"'); res.json({ user: req.user, jobDescriptions, profileVariants, usageEvents, connectionRequests, jobRecommendations, exportedAt: new Date() });
});

router.delete('/account', authenticate, async (req, res) => {
  await Promise.all([JobDescription.deleteMany({ userId: req.user.userId }), ProfileVariant.deleteMany({ userId: req.user.userId }), UsageEvent.deleteMany({ userId: req.user.userId }), ConnectionRequest.deleteMany({ userId: req.user.userId }), JobRecommendation.deleteMany({ userId: req.user.userId }), AuthSession.deleteMany({ userId: req.user.userId }), AccountToken.deleteMany({ userId: req.user.userId })]);
